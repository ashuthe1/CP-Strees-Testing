// Ashutosh Gautam ツ
// tag: special/segmentTree/Distinct_Value_queries
// link: https://youtu.be/7JmBP-RqzlI?feature=shared
...
void AshutoshGautam() {
    int n, q; cin >> n >> q;
    vector<int> v(n+1), ans(q+1), arr(n+1, 0);
    vector<vector<int>> que[n+1];
    map<int, vector<int>> pos; // pos[i] --> stores indexes where i has occured;
    map<int, int> pointer; // pointer[i] --> pointer to occurence of i
    ...
    for(int l = 1; l <= n; l++) {
        for(auto e: que[l]) {
            int r = e[1], idx = e[2];
            int uniqueElements = st.query(l, r).val;
            deb(l, r, uniqueElements)
            ans[idx] = uniqueElements;
        }
        if(arr[l] == 0) continue;
        pointer[v[l]]++;
        int nextIdx = pointer[v[l]];
        if(nextIdx < pos[v[l]].size()) {
            int idxToUpdate = pos[v[l]][nextIdx];
            st.rangeUpdate(idxToUpdate, idxToUpdate, 1);
            arr[idxToUpdate] = 1;
        }
    }
